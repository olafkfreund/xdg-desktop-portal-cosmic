use crate::{
    access, config, file_chooser, remotedesktop_dialog, screencast_dialog, screenshot,
    subscription,
};
use cosmic::Task;
...
pub struct CosmicPortal {
    ...
    pub remotedesktop_args: Option<remotedesktop_dialog::Args>,
...
pub enum Msg {
    ...
    RemoteDesktop(remotedesktop_dialog::Msg),
...
impl cosmic::Application for CosmicPortal {
    ...
    fn view_window(&self, id: window::Id) -> cosmic::Element<'_, Self::Message> {
        ...
        } else if id == *remotedesktop_dialog::REMOTEDESKTOP_ID {
            remotedesktop_dialog::view(self).map(Msg::RemoteDesktop)
        ...
        match message {
            ...
            Msg::Portal(e) => match e {
                ...
                subscription::Event::RemoteDesktop(args) => {
                    remotedesktop_dialog::update_args(self, args).map(cosmic::Action::App)
                }
                subscription::Event::CancelRemoteDesktop(handle) => {
                    remotedesktop_dialog::cancel(self, handle).map(cosmic::Action::App)
                }
            ...
            Msg::RemoteDesktop(m) => {
                remotedesktop_dialog::update_msg(self, m).map(cosmic::Action::App)
            }
