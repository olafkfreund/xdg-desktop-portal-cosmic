...
use crate::remotedesktop_dialog::{self, CaptureSources};
...
impl RemoteDesktop {
    ...
    async fn start(
        ...
        let on_cancel =
            || remotedesktop_dialog::hide_remotedesktop_prompt(&self.tx, &session_handle);
...
fn create_eis_socket_pair() -> io::Result<(std::os::fd::OwnedFd, zvariant::OwnedFd)> {
    ...
    Ok((std::os::fd::OwnedFd::from(server), zvariant::OwnedFd::from(std::os::fd::OwnedFd::from(client))))
}
